...
class BYOL(nn.Module):
    def __init__(
        ...
        #self.forward(torch.randn(2, 3, image_size, image_size, device=self.device))
    ...
    def forward(
        ...
        with torch.no_grad():
            self.target_encoder = self._get_target_encoder() if self.use_momentum else self.online_encoder
            self.target_encoder = self.target_encoder.to(self.device)
            
            target_projections, _ = self.target_encoder(images)
    ...
    def train_one_epoch(self, positive1, positive2, opt):
        
        opt.zero_grad()
        
        loss = self.forward(positive1, positive2)
